...
if switching:
    sw_rows = []
    for pair_key, pair_data in switching.items():
        if isinstance(pair_data, dict):
            for param, sv_data in pair_data.items():
                if isinstance(sv_data, dict) and "switching_value" in sv_data:
                    sw_rows.append({
                        "Scenario Pair": pair_key.replace("_vs_", " vs. ").replace("_", " ").title(),
                        "Parameter": param.replace("_", " ").title(),
                        "Base Value": sv_data.get("base_value", "—"),
                        "Switching Value": sv_data.get("switching_value", "—"),
                        "Change Required": f"{sv_data.get('pct_change', 0)*100:+.0f}%" if sv_data.get("pct_change") else "—",
                    })
    
    if sw_rows:
        sw_df = pd.DataFrame(sw_rows).head(20)
        from IPython.display import Markdown
        Markdown(sw_df.to_markdown(index=False))
    else:
        print("Switching values available but in different format. See Appendix A for full analysis.")
else:
    ...
    if isinstance(h_data, dict):
        ...
        for s in ALT_SCENARIOS:
            ...
            if isinstance(s_data, dict):
                vals.append(s_data.get("npv_savings", s_data.get("npv", 0)) / 1e9)
            else:
                vals.append(0)
        fig.add_trace(go.Bar(
            name=h_label,
...
if ddr:
    fig = go.Figure()
    scenarios_in_ddr = [s for s in SCENARIO_IDS if s in ddr]
    
    if scenarios_in_ddr:
        constant_vals = [cba["npv_results"][s]["pv_total_costs"] / 1e9 for s in scenarios_in_ddr]
        ddr_vals = [ddr[s].get("pv_total_costs", 0) / 1e9 for s in scenarios_in_ddr]
        labels = [SCENARIO_SHORT[s] for s in scenarios_in_ddr]
        
        fig.add_trace(go.Bar(name=f"Constant {cba['discount_rate']*100:.0f}%", x=labels, y=constant_vals, marker_color="#3498db"))
        fig.add_trace(go.Bar(name=f"DDR ({cba['declining_discount_rate']['ddr_schedule']['years_0_30']*100:.1f}%→{cba['declining_discount_rate']['ddr_schedule']['years_31_75']*100:.1f}%→{cba['declining_discount_rate']['ddr_schedule']['years_76_125']*100:.1f}%)", x=labels, y=ddr_vals, marker_color="#e74c3c"))
        ...
        print("DDR data available but scenario keys don't match. See outputs for raw data.")
else:
