...
This appendix lists every parameter in the CBA model. All values come from `parameters.csv`, the single source of truth. Low and High columns define sensitivity and Monte Carlo ranges, derived from the literature rather than arbitrary percentage bands.

Parameters are grouped by category (demand, technology, economics, etc.). Each carries a source citation.
...
CBA credibility rests on input data quality. Peer-reviewed and institutional sources (IRENA, IEA, ADB, World Bank, IPCC) outweigh assumed values. The chart below classifies each parameter by source quality.
...
roadmap_data = [
    ...
    {"Category": "Demand", "Data Point": "Malé peak demand", "Roadmap Value": "107 MW (PNG)", "Model Value": f"{param('Base Peak 2026'):.0f} MW (national)", "Match": "✅", "Note": "Model includes outer islands; Malé = ~53%"},
    ...
    {"Category": "Supply", "Data Point": "Fuel efficiency (outer)", "Roadmap Value": "0.42 L/kWh (2.38 kWh/L)", "Model Value": f"{param('Fuel Efficiency', category='Fuel'):.1f} kWh/L", "Match": "✅", "Note": "National average higher; outer islands less efficient"},
    {"Category": "Solar", "Data Point": "Existing PV installed", "Roadmap Value": "68.5 MW (as of 2023)", "Model Value": f"{param('Solar PV Capacity', category='Current System'):.0f} MW", "Match": "✅", "Note": "Direct match"},
    ...
    {"Category": "Grid", "Data Point": "Outer island grid losses", "Roadmap Value": "12%", "Model Value": f"{param('Distribution Loss', category='Losses')*100:.0f}%", "Match": "✅", "Note": "National weighted average ≈ 11%"},
    ...
    {"Category": "Fiscal", "Data Point": "Annual electricity subsidy", "Roadmap Value": "MVR 3B (~$200M/yr)", "Model Value": f"${param('Current Subsidy per kWh', category='Macro')*1200:.0f}M/yr", "Match": "✅", "Note": "$0.15/kWh × 1,200 GWh ≈ $180M (close match)"},
    {"Category": "RE Target", "Data Point": "2028 RE target", "Roadmap Value": "33% of consumption", "Model Value": f"{param('Domestic RE Target 2050', category='RE Deployment')*100:.0f}% (2050 target)", "Match": "⚠️", "Note": "Model targets 50% by 2050; Roadmap targets 33% by 2028"},
]
